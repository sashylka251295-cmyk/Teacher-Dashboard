...
6. Optionally add homework, student-facing feedback, or a goal update.
7. Save the update.

Student-facing feedback may be linked to several objectives from the same update through `learningTargetIds`.
...
- Safe current target fields are copied to `students/{studentId}.independentLearning`; draft feedback is never copied there.
